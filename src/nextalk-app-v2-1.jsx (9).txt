  ...
  const registerWithEmail = async () => {
    ...
    try {
      ...
      if (firebaseDB && window.firebase_ref && window.firebase_get) {
        ...
        if (usernameSnap.exists()) { setError("اسم المستخدم ده موجود بالفعل، جرب اسم تاني"); setLoading(false); return; }
      }
      ...
      else setError(e.message || "حدث خطأ، حاول مرة تانية");
    }
    setLoading(false);
  };
...
export default function NexTalkApp() {
          ...
          if (fireUser) {
            try {
              ...
              if (snap.exists()) {
                ...
                // Google user not saved yet — save them now
                const userData = {
                  uid: fireUser.uid,
                  name: fireUser.displayName || "User",
                  phone: fireUser.phoneNumber || "",
              ...
              // else: phone user not saved yet — LoginScreen will handle via onLogin
            } catch (e) {
  ...
  // Step 2: Auth
  if (!user) return <LoginScreen onLogin={(userData) => { setUser(userData); if (userData.lang) setLang(userData.lang); }} lang={lang} />;
