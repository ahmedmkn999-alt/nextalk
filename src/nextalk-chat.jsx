...
function ChatWindow({ chat, currentUser, onBack, isMobile }) {
  ...
  useEffect(() => {
    ...
    if (window.firebase_ref && window.firebase_onValue && window._firebaseDB) {
      ...
      const unsub = window.firebase_onValue(ref, async snap => {
        ...
        // Mark as read
        if (window.firebase_update && window.firebase_ref && window._firebaseDB) {
    ...
    try {
      ...
      if (window.firebase_ref && window.firebase_set && window.firebase_update && window._firebaseDB) {
        ...
        await window.firebase_update(window.firebase_ref(window._firebaseDB,`chats/${chat.id}`), {
          ...
          lastTs: Date.now(),
          [`unread_${chat.userId}`]: (chat.unread || 0) + 1,
        });
  ...
  const handleReact = async (msgId, emoji) => {
    ...
    if(window.firebase_ref&&window.firebase_get&&window.firebase_set&&window._firebaseDB){
