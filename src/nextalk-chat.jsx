  ...
  return (
    <div style={{ position: "absolute", bottom: "100%", left: 0, marginBottom: 8, background: C.card, border: `1px solid ${C.border}`, borderRadius: 16, padding: 12, zIndex: 999, boxShadow: `0 8px 40px #00000088`, animation: "popIn 0.15s ease" }}>
      <div style={{ display: "grid", gridTemplateColumns: "repeat(6, 1fr)", gap: 4 }}>
        {EMOJIS.map(e => (
          <button key={e} onClick={() => { onPick(e); onClose(); }} style={{ background: "none", border: "none", fontSize: 22, cursor: "pointer", padding: "4px", borderRadius: 8, transition: "background 0.1s" }}
      ...
      <div style={{ maxWidth: "72%", position: "relative" }}>
        ...
        {msg.reactions && Object.keys(msg.reactions).length > 0 && (
          <div style={{ display: "flex", flexWrap: "wrap", gap: 4, marginTop: 4, justifyContent: isMe ? "flex-end" : "flex-start" }}>
            {Object.entries(msg.reactions).map(([emoji, count]) => (
              <button key={emoji} onClick={() => onReact(msg.id, emoji)} style={{ background: C.surface, border: `1px solid ${C.border}`, borderRadius: 12, padding: "2px 7px", fontSize: 12, cursor: "pointer", color: C.text }}>
                {emoji} {count}
              </button>
            ))}
          </div>
        ...
        {hov && (
          <div style={{
            ...
            borderRadius: 12, padding: "4px 6px", zIndex: 10,
            boxShadow: "0 4px 20px #00000066", animation: "fadeIn 0.15s ease",
          }}>
        ...
        {showReactions && (
          <div style={{
            position: "absolute", top: -72, [isMe ? "left" : "right"]: 0, zIndex: 11,
            background: C.card, border: `1px solid ${C.border}`, borderRadius: 30,
...
function ChatWindow({ chat, currentUser, onBack, isMobile }) {
  ...
  useEffect(() => {
    ...
    if (window.firebase_ref && window.firebase_onValue && window._firebaseDB) {
      ...
      setMessages([
        ...
        { id: "3", senderId: chat.userId, senderName: chat.name, text: "Totally agree! The UI is 🔥", ts: Date.now() - 180000, read: true },
        { id: "4", senderId: currentUser.uid, senderName: currentUser.name, text: "Thanks! Built with ❤️", ts: Date.now() - 60000, read: false },
      ]);
    }
  }, [chat?.id]);
  ...
  useEffect(() => {
    if (!chat) return;
    const t = setTimeout(() => setIsTyping(Math.random() > 0.7), 3000);
    return () => clearTimeout(t);
  }, [messages]);

  const sendMessage = async (extraText = null, imageUrl = null) => {
    const msgText = extraText ?? text.trim();
    ...
    if (window.firebase_ref && window.firebase_set && window._firebaseDB) {
      ...
      await window.firebase_set(window.firebase_ref(window._firebaseDB, `chats/${chat.id}/messages/${key}`), msg);
      await window.firebase_update(window.firebase_ref(window._firebaseDB, `chats/${chat.id}`), {
        lastMessage: msgText || "📷 Photo",
        lastTs: Date.now(),
      });
    } else {
  ...
  const handleReact = async (msgId, emoji) => {
    ...
    if (window.firebase_ref && window._firebaseDB) {
      ...
      window.firebase_set(window.firebase_ref(window._firebaseDB, path), cur + 1);
    }
  };
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", height: "100%", background: C.bg }}>
      ...
      <div style={{ padding: "12px 16px", background: C.surface, borderBottom: `1px solid ${C.border}`, display: "flex", alignItems: "center", gap: 12, flexShrink: 0 }}>
        {isMobile && (
          <button onClick={onBack} style={{ background: "none", border: "none", color: C.accent, cursor: "pointer", fontSize: 18, padding: 0 }}>←</button>
        )}
        ...
        <div style={{ flex: 1 }}>
          ...
          <div style={{ fontSize: 11, color: chat.online ? C.green : C.muted }}>
            {isTyping && chat.online ? "✍️ typing..." : chat.online ? "online" : "last seen recently"}
          </div>
        </div>
        <button style={{ background: C.accentDim, border: `1px solid ${C.accent}33`, borderRadius: 10, padding: "6px 10px", color: C.accent, cursor: "pointer", fontSize: 16 }}>📞</button>
        <button style={{ background: C.purpleDim, border: `1px solid ${C.purple}33`, borderRadius: 10, padding: "6px 10px", color: C.purple, cursor: "pointer", fontSize: 16 }}>📹</button>
      </div>
        ...
        {Object.entries(grouped).map(([day, msgs]) => (
          <div key={day}>
            <div style={{ textAlign: "center", margin: "12px 0 8px" }}>
              <span style={{ background: C.surface, border: `1px solid ${C.border}`, borderRadius: 20, padding: "4px 14px", fontSize: 11, color: C.muted }}>{day === new Date().toDateString() ? "Today" : day}</span>
            </div>
            {msgs.map(m => (
              <MessageBubble key={m.id} msg={m} isMe={m.senderId === currentUser.uid} onReact={handleReact} onReply={setReplyTo} onDelete={handleDelete} />
            ))}
          </div>
        ))}
        <div ref={bottomRef} />
      ...
      {replyTo && (
        <div style={{ padding: "8px 16px", background: C.surface, borderTop: `1px solid ${C.border}`, display: "flex", alignItems: "center", gap: 10 }}>
          <div style={{ flex: 1, borderLeft: `3px solid ${C.accent}`, paddingLeft: 10, fontSize: 12 }}>
            ...
            <div style={{ color: C.muted }}>{replyTo.text?.slice(0, 60)}…</div>
          </div>
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", height: "100%", background: C.surface, borderRight: `1px solid ${C.border}` }}>
      ...
      <div style={{ flex: 1, overflowY: "auto" }}>
        {filtered.length === 0 && (
          <div style={{ textAlign: "center", padding: 32, color: C.muted, fontSize: 13 }}>No conversations yet</div>
        )}
        {filtered.map(chat => (
          <div
            key={chat.id}
          ...
          >
            ...
            <div style={{ flex: 1, minWidth: 0 }}>
              ...
              <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                ...
                {chat.unread > 0 && (
                  <div style={{ background: C.accent, color: "#000", borderRadius: 10, padding: "1px 7px", fontSize: 10, fontWeight: 800, flexShrink: 0 }}>{chat.unread}</div>
                )}
              </div>
            </div>
          </div>
    ...
    <div style={{ height: "100%", display: "flex", overflow: "hidden" }}>
      ...
      {showList && (
        <div style={{ width: isMobile ? "100%" : 320, flexShrink: 0, height: "100%" }}>
          <ChatList chats={chats} activeId={activeChat?.id} onSelect={setActiveChat} currentUser={currentUser} isMobile={isMobile} />
        </div>
      )}
        ...
        <div style={{ flex: 1, height: "100%", display: "flex", flexDirection: "column" }}>
          {activeChat ? (
            ...
            <div style={{ flex: 1, display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", color: C.muted, gap: 16 }}>
              ...
              <div style={{ fontSize: 13 }}>Choose a chat from the list to start messaging</div>
            </div>
          )}
        </div>
      )}
