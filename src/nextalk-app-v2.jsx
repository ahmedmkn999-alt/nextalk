        ...
        <div style={{
          ...
          {step === "phone" && (
            <>
              ...
              <div style={{ display: "flex", alignItems: "center", gap: 12, margin: "16px 0" }}>
                <div style={{ flex: 1, height: 1, background: C.border }} />
                <span style={{ color: C.muted, fontSize: 12, fontWeight: 600, letterSpacing: 1 }}>لديك حساب؟</span>
                <div style={{ flex: 1, height: 1, background: C.border }} />
              </div>

              {/* Login with Email Button */}
              <button onClick={() => { setStep("login-email"); setError(""); }} disabled={loading} style={{
                width: "100%", padding: "14px", borderRadius: 14, border: `1px solid ${C.green}66`,
                background: `linear-gradient(135deg, ${C.greenDim}, #00ff8810)`,
                color: C.green, fontSize: 15, fontWeight: 700,
                cursor: loading ? "not-allowed" : "pointer",
                display: "flex", alignItems: "center", justifyContent: "center", gap: 10,
                transition: "all 0.2s", opacity: loading ? 0.5 : 1,
              }}
                onMouseEnter={e => { if (!loading) { e.currentTarget.style.background = `${C.green}25`; e.currentTarget.style.transform = "translateY(-2px)"; }}}
                onMouseLeave={e => { e.currentTarget.style.background = `linear-gradient(135deg, ${C.greenDim}, #00ff8810)`; e.currentTarget.style.transform = "translateY(0)"; }}
              >
                <span style={{ fontSize: 18 }}>📧</span>
          ...
          {step === "login-email" && (
            <>
              ...
              <div style={{ textAlign: "center", marginBottom: 24 }}>
                <div style={{ fontSize: 36, marginBottom: 8 }}>📧</div>
                <div style={{ fontSize: 20, fontWeight: 800, marginBottom: 4 }}>تسجيل الدخول</div>
                <div style={{ color: C.muted, fontSize: 13 }}>أدخل بياناتك للدخول على حسابك</div>
              </div>
              ...
              <button onClick={loginWithEmail} disabled={loading} style={{
                ...btnPrimary(),
                background: `linear-gradient(135deg, ${C.green}, #00cc66)`,
              ...
              <div style={{ textAlign: "center", marginTop: 16 }}>
                ...
                <button onClick={() => { setStep("register"); setError(""); }} style={{
                  background: "none", border: "none", color: C.accent, cursor: "pointer", fontSize: 13, fontWeight: 700, padding: 0,
...
export default function NexTalkApp() {
  ...
  useEffect(() => {
    // Load Firebase + Dashboard together
    const loadFirebase = async () => {
      try {
        const [
          ...
          import("./nextalk-dashboard-v2"),
        ]);
        setAdminDashboard(() => dashModule.default);
        ...
        onAuthStateChanged(firebaseAuth, async (fireUser) => {
          if (fireUser) {
            try {
              ...
              if (!snap.exists()) {
                // Wait for DB write (registration race condition)
              ...
              // email/phone: onLogin() in LoginScreen already called setUser — just setLoading false
            } catch (e) {
  ...
  // Step 3: Dashboard
  if (!AdminDashboard) {
    // Still loading dashboard module — trigger load if not started
