        ...
        <div style={{ background: C.surface, border: `1px solid ${C.border}`, borderRadius: 24, padding: "28px 24px" }}>
          ...
          {step === "main" && (
            <>
              ...
              <button onClick={() => setStep("phoneInput")} disabled={loading} style={{ ...btnBlue, display: "flex", alignItems: "center", justifyContent: "center", gap: 10, marginBottom: 10 }}>
                <span>📱</span> دخول برقم التليفون
              </button>
...
export default function NexTalkApp() {
  ...
  useEffect(() => {
    ...
    // Listen for auth state — this is instant since Firebase is npm, not CDN
        ...
        try {
          ...
          if (snap.exists()) {
            ...
            const userData = {
              ...
              photoURL: fireUser.photoURL || null, email: fireUser.email || null,
              premium: false, badge: null, joinedAt: new Date().toISOString(),
              isAdmin: false, lastLogin: Date.now(), twoFA: { enabled: false }, authProvider: "google",
            };
            await set(ref(firebaseDB, `users/${fireUser.uid}`), userData);
            setUser(prev => prev || userData);
            saveLang(lang || "ar");
