...
export default function NexTalkApp() {
  ...
  const [lang, setLang] = useState(() => localStorage.getItem("nextalk_lang") || null);
  const [AdminDashboard, setAdminDashboard] = useState(null);
  const [ChatScreen, setChatScreen] = useState(null);

  useEffect(() => {
    // Safety net: release loading after 5s no matter what
    const safetyTimeout = setTimeout(() => setLoading(false), 5000);

    const loadFirebase = async () => {
      // Helper: import with timeout
      const importWithTimeout = (url, ms = 6000) => Promise.race([
        import(url),
        new Promise((_, reject) => setTimeout(() => reject(new Error("Import timeout: " + url)), ms)),
      ]);
      try {
        const [
          ...
          importWithTimeout("https://www.gstatic.com/firebasejs/10.8.0/firebase-app.js"),
          importWithTimeout("https://www.gstatic.com/firebasejs/10.8.0/firebase-auth.js"),
          importWithTimeout("https://www.gstatic.com/firebasejs/10.8.0/firebase-database.js"),
        ]);
            ...
            try {
              ...
              if (snap.exists()) {
                ...
                setUser(prev => {
                  ...
                  if (userData.lang) { setLang(userData.lang); localStorage.setItem("nextalk_lang", userData.lang); }
  ...
  if (!lang) return <LanguagePicker onSelect={(l) => { localStorage.setItem("nextalk_lang", l); setLang(l); }} />;

  // Step 2: Auth
  if (!user) return <LoginScreen onLogin={(userData) => { setUser(userData); const l = userData.lang || lang; setLang(l); localStorage.setItem("nextalk_lang", l); setLoading(false); }} lang={lang} />;
