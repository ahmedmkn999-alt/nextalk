...
function LoginScreen({ onLogin, lang }) {
  ...
  const signInWithGoogle = async () => {
    setLoading(true); setError("");
    try {
      if (firebaseAuth && window.firebase_GoogleAuthProvider && window.firebase_signInWithPopup) {
        ...
        if (!userData) {
          // New user — save profile from Google
          userData = {
            uid: fireUser.uid,
          ...
          {step === "phone" && (
            <>
              ...
              <button onClick={signInWithGoogle} disabled={loading} style={{
                ...
                border: `1px solid ${C.border}`,
                background: loading ? C.surface : "rgba(255,255,255,0.05)",
                color: C.text, fontSize: 15, fontWeight: 700, cursor: loading ? "not-allowed" : "pointer",
                display: "flex", alignItems: "center", justifyContent: "center", gap: 12,
                transition: "all 0.2s", opacity: loading ? 0.5 : 1,
                boxShadow: "none",
              }}
                onMouseEnter={e => { if (!loading) { e.currentTarget.style.background = "rgba(255,255,255,0.1)"; e.currentTarget.style.borderColor = "#ffffff44"; }}}
    ...
    const loadFirebase = async () => {
      try {
        const [
          ...
          { getAuth, onAuthStateChanged, signInWithPhoneNumber, RecaptchaVerifier, GoogleAuthProvider, signInWithPopup },
